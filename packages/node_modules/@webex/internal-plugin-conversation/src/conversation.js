...
const Conversation = WebexPlugin.extend({
   ...
   * @returns {Promise}
   */
  async acknowledge(conversation, object, activity) {
    const url = await this.getConvoUrl(conversation);
   ...
   */
  async add(conversation, participant, activity) {
    const url = await this.getConvoUrl(conversation);
    const convoWithUrl = Object.assign({}, conversation, {url});
   ...
   * @returns {Promise}
   */
  async delete(conversation, object, activity) {
    const url = await this.getConvoUrl(conversation);
    const convoWithUrl = Object.assign({}, conversation, {url});
  ...
  */
   ...
   * @returns {Promise<Conversation>}
   */
  async get(conversation, options = {}) {
    const {user} = conversation;
    let urlPromise;

    try {
      const url = !user ? await this.getConvoUrl(conversation) : '';

      urlPromise = Promise.resolve({url});
    }
    catch (noUrl) {
      urlPromise = this._inferConversationUrl(conversation);
    }
      ...
      .then(({url}) => {
        ...
        return Promise.resolve(user ? this.webex.internal.user.asUUID(user) : null)
          .then((userId) => {
            ...
            /*if (params.uri.startsWith('https://conv.a6.ciscospark.com')) {
              params.uri = 'http://127.0.0.1:3000/ping' + params.uri.substring(30);
            }*/
   ...
   * @returns {Promise<Activity>}
   */
  async leave(conversation, participant, activity) {
    const convoWithUrl =
      Object.assign(
        {}, conversation, {url: await this.getConvoUrl(conversation)}
      );
   ...
   * @returns {Promise}
   */
  async cardAction(conversation, inputs, parentActivity, activity = {}) {
    const convoWithUrl =
      Object.assign(
        {}, conversation, {url: await this.getConvoUrl(conversation)}
      );
   ...
   * @returns {Promise<Activity>}
   */
  async post(conversation, message, activity) {
    const convoWithUrl =
      Object.assign(
        {}, conversation, {url: await this.getConvoUrl(conversation)}
      );
   ...
   * @returns {Promise<Activity>}
   */
  async assign(conversation, avatar) {
  ...
  async getConvoUrl({
    ...
    if (id) {
      if (cluster) {
        // TODO: GET THIS TO WORK WITH CLUSTERS
        // This will use the home cluster
        return (await this._inferConversationUrl({id})).url;
      }
      this.logger.warn(
   ...
   * @returns {Promise}
   */
  async updateTypingStatus(conversation, options) {
    if (!conversation.id) {
      if (conversation.url) {
   ...
   * @returns {Promise<Activity>}
   */
  async share(conversation, activity) {
    if (isArray(activity)) {
      activity = {
        object: {
          files: activity
   ...
   * @returns {Promise<Activity>}
   */
  async submit(activity, endpoint) {
    const url = endpoint || await this.getConvoUrl(activity.target);
   ...
   * @returns {Promise}
   */
  async unassign(conversation, activity) {
    const convoWithUrl =
      Object.assign(
        {}, conversation, {url: await this.getConvoUrl(conversation)}
      );
   ...
   * @returns {Promise}
   */
  async update(conversation, object, activity) {
    if (!isObject(object)) {
      return Promise.reject(new Error('`object` must be an object'));
    }
   ...
   * @returns {Promise<Activity>}
   */
  async updateKey(conversation, key, activity) {
    const convoWithUrl =
      Object.assign(
        {}, conversation, {url: await this.getConvoUrl(conversation)}
      );
   ...
   * @returns {Promise<Activity>}
   */
  async _updateKey(conversation, key, activity) {
    const convoWithUrl =
      Object.assign(
        {}, conversation, {url: await this.getConvoUrl(conversation)}
      );
    ...
    return Promise.resolve(
      ...
      .then((keys) => {
        ...
        if (convoWithUrl.defaultActivityEncryptionKeyUrl) {
          params.kmsMessage = {
            method: 'update',
            resourceUri: '<KRO>',
            uri: k.uri
   ...
   * @returns {Promise<Array<Activity>>}
   */
  async _listActivities(options) {
    const id = options.conversationId;
    const url = await this.getConvoUrl({url: options.conversationUrl, id});
...
[
  ...
  Conversation.prototype[verb] = async function submitSimpleActivity(conversation, activity) {
    const convoWithUrl =
...
[
  ...
  Conversation.prototype[verb] = async function submitModerationChangeActivity(conversation, moderator, activity) {
    const convoWithUrl =
      Object.assign(
        {}, conversation, {url: await this.getConvoUrl(conversation)}
      );
...
[
  ...
  Conversation.prototype[fnName] = async function submitSpacePropertyActivity(conversation, tag, activity) {
    if (!isString(tag)) {
...
[
  ...
  Conversation.prototype[verb] = async function submitObjectActivity(conversation, object, activity) {
    if (!isObject(object)) {
      return Promise.reject(new Error('`object` must be an object'));
    }
