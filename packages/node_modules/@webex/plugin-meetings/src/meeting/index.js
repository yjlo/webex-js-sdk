        ...
        switch (newShareStatus) {
          case SHARE_STATUS.REMOTE_SHARE_ACTIVE: {
            ...
            if (this.mediaProperties.mediaDirection?.sendShare) {
              if (oldShareStatus === SHARE_STATUS.WHITEBOARD_SHARE_ACTIVE) {
  ...
  addMedia(options = {}) {
    ...
    return MeetingUtil.validateOptions(options)
      ...
      .then(() => Media.attachMedia(this.mediaProperties, {
        ...
        .then(() => {
          this.mediaRequestSent = true;
          const {peerConnection} = this.mediaProperties;

          console.warn('throwing error: ');
          // After Media Request fail check
          if (!this.count) {
            this.count = true;
      ...
      .catch((error) => {
        console.warn('error began');
        console.warn('cleaning stats analyzer');
        this.statsAnalyzer.stopAnalyzer();
        this.statsAnalyzer = null;

        // 1. Need to clean up the peer connection, 2. clean up the stats analyzer and 3. turn off all listeners
        console.warn('closing peer connection');
        this.mediaProperties.peerConnection.close();
        this.mediaProperties.peerConnection = null;

        // console.warn("check if media request was sent");
        // if (this.mediaRequestSent) {
        //   console.warn("media request was already sent");
        //   this.meetingRequest.deleteMediaRequest(this);
        //   console.warn("clean up complete")
        // }

        LoggerProxy.logger.error(`${LOG_HEADER} Error adding media failed to initiate PC and send request, `, error);

        console.warn('sending operational metrics');
        Metrics.sendOperationalMetric(
          METRICS_OPERATIONAL_MEASURES.ADD_MEDIA_FAILURE,
          {
            correlation_id: this.correlationId,
