...
const Board = WebexPlugin.extend({
   ...
   * @memberof Board.BoardService
   */
  async authorizeMediaInjector(board) {
    const kmsMessage = (
      await this.webex.internal.encryption.kms.prepareRequest({
        method: 'create',
        uri: '/authorizations',
    ...
    const response = await this.webex.request({
        ...
        method: 'PUT',
        body: { kmsMessage: kmsMessage },
    })
    const kmsResponse = response.body.kmsResponse;
    const plaintext = await this.webex.internal.encryption.kms.decryptKmsMessage(kmsResponse);
    // TODO capture authorizations[0].authId, and remove it during deauthorization
    return plaintext.authorizations[0].bearer;
  }
});
