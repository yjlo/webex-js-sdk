...
const Board = WebexPlugin.extend({
  ...
  authorizeMediaInjector(board) {
    return webex.internal.encryption.kms.prepareRequest({
      method: 'create',
      uri: '/authorizations',
      ...
      return this.webex.internal.encryption.kms.decryptKmsMessage(res.body.kmsResponse);
  ...
  unauthorizeMediaInjector(board) {
   return this.webex.internal.encryption.kms.listAuthorizations({
      ...
      for (let i = 0; i < authorizations.length; i++) {
        promises.push(
          this.webex.internal.encryption.kms.removeAuthorization({
            authId: authorizations[i].authId, kroUri: board.kmsResourceUrl
          }).catch((err) => {
